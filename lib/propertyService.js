...
// Função para deletar mídia (detecta automaticamente se é Cloudinary ou Supabase Storage)
const deleteMedia = async (url) => {
    try {
        // Detectar se é URL do Supabase Storage ou Cloudinary
        ...
        if (isSupabaseUrl) {
            ...
            await MediaService.deleteFromSupabase(url, 'properties');
            console.log('✅ Arquivo excluído do Supabase Storage');
        } else if (isCloudinaryUrl) {
            // Deletar do Cloudinary via backend
            ...
            console.warn('⚠️ URL não reconhecida (nem Supabase nem Cloudinary):', url);
        }
    } catch (error) {
...
// Função para deletar do Cloudinary (mantida para compatibilidade)
...
export class PropertyService {
    static async createProperty(propertyData, mediaFiles = [], onProgress = null, videoUrls = []) {
        try {
            ...
            console.log('🔍 DEBUG - propertyData recebido:', propertyData);
            console.log('🔍 DEBUG - Latitude recebida:', propertyData.latitude);
            console.log('🔍 DEBUG - Longitude recebida:', propertyData.longitude);
            console.log('🔍 DEBUG - Tipo da latitude:', typeof propertyData.latitude);
            console.log('🔍 DEBUG - Tipo da longitude:', typeof propertyData.longitude);
            ...
            // Validar coordenadas antes de salvar no banco
            if (property.latitude !== null && property.longitude !== null) {
                const lat = parseFloat(property.latitude);
                const lng = parseFloat(property.longitude);
                
                if (isNaN(lat) || isNaN(lng)) {
                    console.error('❌ ERRO: Coordenadas inválidas no banco:', { lat, lng });
                    throw new Error('Coordenadas inválidas para salvar no banco');
                }
                
                console.log('✅ Coordenadas válidas para salvar:', { lat, lng });
            } else {
                console.warn('⚠️ Coordenadas são null - propriedade será salva sem localização');
            }

            console.log('🔍 DEBUG - Property que será salvo no banco:', property);
            console.log('🔍 DEBUG - Latitude final no banco:', property.latitude);
            console.log('🔍 DEBUG - Longitude final no banco:', property.longitude);
                    ...
                    for (const mediaObj of mediaUrls) {
                        try {
                            ...
                            if (mediaObj && mediaObj.secure_url) {
                                await deleteMedia(mediaObj.secure_url);
                            }
                        } catch (deleteError) {
    ...
    static async updateProperty(propertyId, propertyData, newMediaFiles = [], removeMediaUrls = [], newVideoUrls = [], removeVideoUrls = []) {
        try {
            console.log('🔍 updateProperty chamado com:', {
                propertyId,
                newMediaFilesCount: newMediaFiles.length,
                removeMediaUrlsCount: removeMediaUrls.length,
                newVideoUrlsCount: newVideoUrls.length,
                removeVideoUrlsCount: removeVideoUrls.length,
                newVideoUrls,
                removeVideoUrls
            });

            // Deletar mídias removidas (imagens)
            ...
            // Upload de novas mídias (imagens)
            let newMediaUrls = [];
            ...
            const currentVideoUrls = currentProperty.video_urls || [];

            // Filtrar imagens que não foram removidas
            const remainingImages = currentImages.filter(url => !removeMediaUrls.includes(url));

            // Filtrar vídeos que não foram removidos
            const remainingVideoUrls = currentVideoUrls.filter(url => !removeVideoUrls.includes(url));

            console.log('🎥 Processamento de vídeos:', {
                currentVideoUrls,
                removeVideoUrls,
                remainingVideoUrls,
                newVideoUrls
            ...
            // Combinar vídeos restantes com novos
            const updatedVideoUrls = [...remainingVideoUrls, ...newVideoUrls];

            console.log('🎥 Vídeos finais que serão salvos:', updatedVideoUrls);
            ...
            const updateData = {
                ...
                video_urls: updatedVideoUrls.length > 0 ? updatedVideoUrls : null,
                status: propertyData.status || currentProperty.status, // Incluir status se fornecido
                ad_status: propertyData.ad_status || currentProperty.ad_status, // Incluir ad_status se fornecido
    ...
    static async updateAdStatus(propertyId, adStatus) {
        try {
            if (!['active', 'inactive'].includes(adStatus)) {
                throw new Error('ad_status inválido');
            }

            const { data, error } = await supabase
                .from('properties')
                .update({ ad_status: adStatus, updated_at: new Date().toISOString() })
                .eq('id', propertyId)
                .select('id, ad_status')
                .single();
            ...
            console.error('Erro ao atualizar ad_status:', error);
            throw error;
        }
    }
