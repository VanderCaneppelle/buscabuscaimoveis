...
// Evitar limpezas concorrentes do cache
    ...
    try {
        ...
        for (const [url, info] of entries) {
            ...
            try {
                ...
                // Not found/ENOENT não é erro funcional
                console.log('ℹ️ Arquivo já inexistente ao remover por prioridade:', info.filePath);
            }
        }
...
const getOptimizedUrl = async (url, fileType = 'media') => {
    ...
    // ⚠️ Para vídeos HLS (playlist.m3u8) não fazemos cache local.
    if (fileType === 'video') {
...
const removeFromCache = async (url) => {
    try {
        ...
        if (metadata[url]) {
            ...
            if (fileInfo.exists) {
                await FileSystem.deleteAsync(metadata[url].filePath, { idempotent: true });
        ...
        // Tratar remoção como idempotente
        console.log('ℹ️ Arquivo já inexistente ao remover do cache');
    }
};
...
const cleanupOldCache = async () => {
    if (isCacheCleanupRunning) {
        // Evitar corrida entre múltiplas limpezas
        return;
    }
        ...
        for (const [url, info] of Object.entries(metadata)) {
            ...
            if (isExpired || isOldStory) {
                try {
                    ...
                    // Not found/ENOENT é aceitável aqui
                    console.log('ℹ️ Arquivo expirado já inexistente:', info.filePath);
                }
            }
        }
