...
export class StoryService {
    ...
    static async deleteMediaFromStorage(url) {
        try {
            ...
            } else if (url.includes('.b-cdn.net') || url.includes('mediadelivery.net')) {
                await this.deleteFromBunny(url);
            } else {
 ...
 */
    ...
    /**
     * Excluir vídeo do Bunny.net
     * @param {string} url - URL do vídeo (stream, thumbnail ou embed)
     */
    static async deleteFromBunny(url) {
        try {
            const videoId = BunnyVideoService.extractVideoId(url);

            if (!videoId) {
                console.warn('⚠️ Não foi possível extrair o videoId do Bunny para exclusão:', url);
                return;
            }

            console.log('🗑️ Excluindo vídeo do Bunny.net:', videoId);
            await BunnyVideoService.deleteVideo(videoId);
        } catch (error) {
            console.warn('⚠️ Erro ao excluir vídeo do Bunny.net:', error);
        }
    }
