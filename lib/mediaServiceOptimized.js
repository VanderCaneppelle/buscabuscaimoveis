...
export class MediaServiceOptimized {
    ...
    static async saveStoryToDatabase(publicUrl, title, mediaType, thumbnailUrl = null, linkUrl = null, linkText = null, linkPosition = null, titlePosition = null, linkCoordinates = null, titleCoordinates = null, titleLayout = null, titleScale = null, linkScale = null, userId = null, options = {}) {
        try {
            ...
            if (finalStatus === 'active') {
                ...
                console.log('ℹ️ Story salvo com status não ativo. Notificação será disparada via webhook:', finalStatus);
            }
    ...
    static async uploadStoryWithBunny(fileUri, title = 'Story', mediaType = null, onProgress = null, linkData = null, titlePosition = 'bottom-center', titleCoordinates = null, titleLayout = 'center', titleScale = 1.0, linkScale = 1.0, userId = null) {
        try {
            console.log('═══════════════════════════════════════');
            ...
            // Gerar thumbnail local como backup e salvar no Supabase
            let localThumbnail = bunnyResult.thumbnailUrl;
            if (mediaType === 'video') {
                try {
                    console.log('📸 Gerando thumbnail local como backup...');
                    const thumbnailUri = await this.generateVideoThumbnail(fileUri);
                    
                    // Upload thumbnail para Supabase
                    ...
                    console.log('✅ Thumbnail backup salvo no Supabase:', localThumbnail);
                } catch (thumbError) {
                    console.warn('⚠️ Erro ao gerar thumbnail local, usando do Bunny:', thumbError.message);
                    localThumbnail = bunnyResult.thumbnailUrl;
                }
            }
