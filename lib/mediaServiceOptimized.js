...
export class MediaServiceOptimized {
    ...
    static async uploadToSupabase(fileUri, bucket = 'properties', folder = 'media', onProgress = null) {
        try {
            ...
            if (isVideo) {
                ...
                // Para imagens, usar Supabase Storage
                return await this.uploadImageToSupabase(fileUri, fileName, bucket, mimeType, onProgress);
            }
    ...
    static async uploadSmallFile(fileUri, fileName, bucket) {
        try {
            ...
            if (isVideo) {
                ...
                // Para imagens, usar Supabase Storage
                const result = await this.uploadImageToSupabase(fileUri, fileName, bucket, mimeType);
                return result.secure_url; // Retorna a URL da imagem (compatibilidade)
            }
        } catch (error) {
    ...
    static async uploadImageToSupabase(fileUri, fileName, bucket, mimeType, onProgress = null) {
        try {
            console.log("🖼️ Iniciando upload de imagem para Supabase Storage...");
            ...
            // Notificar progresso de conversão
            if (onProgress) {
                onProgress(50);
            }
            ...
            while (retries > 0) {
                try {
                    console.log(`🔄 Tentativa ${4 - retries}/3 de upload de imagem...`);
                    ...
                    console.log('✅ Imagem enviada com sucesso para Supabase Storage:', publicUrl);
                    
                    // Retornar no formato compatível (objeto com secure_url para manter compatibilidade)
                    return {
                    ...
                    if (retries > 0) {
                        console.warn(`⚠️ Tentativa de upload de imagem falhou, tentando novamente... (${retries} restantes)`);
                        console.warn(`⚠️ Erro:`, error.message);
                        await new Promise(resolve => setTimeout(resolve, 2000));
                    }
                }
            ...
            console.error("❌ Erro no upload de imagem para Supabase Storage:", error);
            throw error;
        }
    }

    // Upload de imagens direto para Cloudinary (DEPRECATED - mantido para compatibilidade)
    ...
    static async deleteFromSupabase(fileUrl, bucket = 'properties') {
        try {
            ...
            // Se for URL completa do Supabase, extrair apenas o caminho
            if (fileUrl.includes('/storage/v1/object/public/')) {
                const parts = fileUrl.split('/storage/v1/object/public/');
                if (parts.length > 1) {
                    // Remover o nome do bucket do início do caminho
                    filePath = parts[1].replace(`${bucket}/`, '');
                }
            } else if (fileUrl.includes(bucket + '/')) {
                // Se já contém o bucket no caminho, remover
                filePath = fileUrl.split(bucket + '/')[1];
            } else {
                // Se for apenas o nome do arquivo, assumir que está na pasta 'media'
                filePath = fileUrl.includes('/') ? fileUrl : `media/${fileUrl}`;
            }
            
            console.log('🗑️ Deletando arquivo do Supabase Storage:', { bucket, filePath });
            ...
            console.log('✅ Arquivo deletado com sucesso do Supabase Storage');
        } catch (error) {
    ...
    static async saveStoryToDatabase(publicUrl, title, mediaType, thumbnailUrl = null, linkUrl = null, linkText = null, linkPosition = null, titlePosition = null, linkCoordinates = null, titleCoordinates = null, titleLayout = null, titleScale = null, linkScale = null, userId = null, options = {}) {
        try {
            ...
            console.log('👤 saveStoryToDatabase - userId:', userId);

            // Normalizar URLs (aceitar string ou objeto Cloudinary)
            const resolvedMediaUrl = typeof publicUrl === 'string'
                ? publicUrl
                : publicUrl?.secure_url || publicUrl?.url || null;

            const resolvedThumbnailUrl = typeof thumbnailUrl === 'string'
                ? thumbnailUrl
                : thumbnailUrl?.secure_url || thumbnailUrl?.url || null;

            if (!resolvedMediaUrl) {
                throw new Error('URL da mídia inválida ou não encontrada ao salvar o story');
            }
            ...
            const bunnyVideoId = options.bunnyVideoId || null;
            const encodingMetadata = options.encodingMetadata || null;
            const finalStatus = statusOverride || 'active';
            const videoMp4Url = options.videoMp4Url || null;
            const notificationTitleRaw = options.notificationTitle;
            const notificationTitle = typeof notificationTitleRaw === 'string'
                ? notificationTitleRaw.trim() || null
                : null;
            ...
            if (finalStatus !== 'active') {
                console.log('ℹ️ Story salvo com status não ativo. Notificação será disparada via trigger/webhook:', finalStatus);
            }
    ...
    static async uploadStory(fileUri, title = 'Story', mediaType = null, onProgress = null, linkData = null, titlePosition = 'bottom-center', titleCoordinates = null, titleLayout = 'center', titleScale = 1.0, linkScale = 1.0, userId = null, extraOptions = {}) {
        try {
            ...
            const fileInfo = await FileSystem.getInfoAsync(fileUri);
            const maxSize = 500 * 1024 * 1024; // 500MB (atualizado para limite do Supabase)
            if (fileInfo.size > maxSize) {
                throw new Error(`Arquivo muito grande. Máximo: 500MB. Tamanho: ${(fileInfo.size / 1024 / 1024).toFixed(2)}MB`);
            }
    ...
    static async uploadStoryWithBunny(fileUri, title = 'Story', mediaType = null, onProgress = null, linkData = null, titlePosition = 'bottom-center', titleCoordinates = null, titleLayout = 'center', titleScale = 1.0, linkScale = 1.0, userId = null, extraOptions = {}) {
        try {
            console.log('═══════════════════════════════════════');
            console.log('🐰 Tentando upload com Bunny.net...');
            console.log('═══════════════════════════════════════');

            // Importar BunnyVideoService dinamicamente
            const BunnyVideoService = require('./bunnyVideoService').BunnyVideoService;
            ...
            // Se não for vídeo, delegar para uploadStory (Supabase) e sair
            if (mediaType !== 'video') {
                console.log('ℹ️ Mídia não é vídeo. Usando fluxo padrão Supabase/Cloudinary.');
            ...
            // Validar tamanho (10GB = 10240MB)
            if (fileSizeMB > 10240) {
                throw new Error(`Arquivo muito grande para Bunny.net. Máximo: 10GB. Tamanho: ${fileSizeMB.toFixed(2)}MB`);
            }

            // Upload para Bunny.net
            const bunnyResult = await BunnyVideoService.uploadComplete(
                fileUri,
            ...
            console.log('🔗 Video URL:', bunnyResult.videoUrl);
            console.log('🖼️ Thumbnail:', bunnyResult.thumbnailUrl);

            // Thumbnail principal: usar a gerada automaticamente pela Bunny
            const primaryThumbnail = bunnyResult.thumbnailUrl;

            // Salvar no banco de dados
            const dbResult = await this.saveStoryToDatabase(
                bunnyResult.videoUrl,
                title,
                mediaType,
                primaryThumbnail,
                linkData?.url,
            ...
            console.log('═══════════════════════════════════════');
            console.log('✅ SUCESSO COM BUNNY.NET!');
            console.log('═══════════════════════════════════════');

            return {
                publicUrl: bunnyResult.videoUrl,
                ...
                storyId: dbResult.storyId
            };

        } catch (error) {
            console.log('═══════════════════════════════════════');
            console.warn('⚠️ Bunny.net falhou!');
            console.error('Erro:', error.message);
            console.log('🔄 Usando Cloudinary como fallback...');
            console.log('═══════════════════════════════════════');

            // Fallback para Cloudinary (método original uploadStory)
            return await this.uploadStory(
                fileUri,
                title,
                mediaType,
