...
export const PushNotificationService = {
    ...
    validateAndRefreshToken: async (userId) => {
        try {
            ...
            if (stored) {
                ...
                if (!PushNotificationService.isTokenExpired(tokenData)) {
            ...
            if (needsNewToken) {
                ...
                if (newToken) {
                    ...
                    // Armazenar localmente com sessionId
                    if (currentSessionId) {
                        await PushNotificationService.storeTokenWithSession(newToken, currentSessionId);
                    } else {
                        await PushNotificationService.storeTokenLocally(newToken);
                    }

                    // Registrar no backend
                    const registered = await PushNotificationService.registerDeviceToken(newToken, userId);
            ...
            // Retornar o token armazenado se ainda válido
            if (stored) {
                const tokenData = JSON.parse(stored);
                return tokenData.token;
            }
    ...
    isCurrentTokenValid: async () => {
        try {
            ...
            const tokenData = JSON.parse(stored);
            return !PushNotificationService.isTokenExpired(tokenData);
        } catch (error) {
    ...
    // Validar token usando sessionId (integração com SessionManager)
    validateTokenWithSession: async (userId) => {
        try {
            console.log('🔄 Validando token usando sessionId...');

            // Verificar se há sessão ativa
            const hasValidSession = await sessionManager.isSessionValid();

            if (!hasValidSession) {
                console.log('⚠️ Sessão inválida - gerando novo token');
                return await PushNotificationService.validateAndRefreshToken(userId);
            }

            // Sessão válida - verificar se token existe e não está expirado
            const stored = await AsyncStorage.getItem('expo_push_token');
            ...
            console.log('✅ Token válido e sessão ativa');
            return tokenData.token;

        } catch (error) {
            console.error('❌ Erro na validação com sessionId:', error);
            // Fallback para validação normal
            return await PushNotificationService.validateAndRefreshToken(userId);
        }
    },

    // Armazenar token com sessionId
    storeTokenWithSession: async (token, sessionId) => {
        try {
            const tokenData = {
                token,
                sessionId,
            ...
            await AsyncStorage.setItem('expo_push_token', JSON.stringify(tokenData));
            console.log('💾 Token armazenado com sessionId:', sessionId);
            return true;
        } catch (error) {
            console.error('❌ Erro ao armazenar token com sessionId:', error);
            return false;
        }
    }
}; 