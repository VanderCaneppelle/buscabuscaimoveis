...
function normalizeFilters(filters = {}) {
    ...
    return { city, propertyType: normalizedPropertyType, minPrice, maxPrice };
}
...
class PropertyCacheService {
    ...
    static async getProperties(options = {}) {
        ...
        try {
            ...
            const cacheValid = await this.isCacheValid(filters, searchTerm, sortOption, page);
            ...
            if (!forceRefresh && hasCached) {
                ...
                // Como a paginação e ordenação agora são feitas no servidor e a chave de cache inclui a página e o sort,
                // os dados em cache JÁ representam a página correta e ordenada. Não reordenar/repaginar aqui.
                const paginatedData = cachedData;

                if (enableParallelUpdate && !cacheValid) {
                    this.updateCacheInBackground(filters, searchTerm, sortOption, page, pageSize).catch(error => {
