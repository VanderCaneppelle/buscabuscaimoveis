...
export class PlanService {
     ...
     * @param {string} endDate - Data de vencimento no formato ISO
     * @returns {boolean} - true se vencido, false se ainda válido
     */
    static isPlanExpiredByDate(endDate) {
    ...
    static async getUserActivePlan(userId) {
        try {
            ...
            if (error) {
                console.warn('⚠️ RPC get_user_active_plan falhou, usando fallback:', error.message);
                // Não lançar erro, continuar para o fallback
            }
            ...
            if (Array.isArray(data) && data.length > 0) {
                ...
                if (directError) {
                    ...
                    // Usar nova lógica de verificação de expiração (apenas data, não hora)
                    const isExpired = directData.end_date && PlanService.isPlanExpiredByDate(directData.end_date);
                    console.log('📅 Verificação de expiração (nova lógica):', {
    ...
    static async getUserPlanSnapshot(userId) {
        try {
            ...
            const { data, error } = await supabase
                ...
                `)
                .eq('user_id', userId)
                .in('status', ['active', 'expired', 'cancelled']) // ✅ Buscar também expirados
                .order('created_at', { ascending: false })
                .limit(1)
                .single();

            if (error) {
                // PGRST116 = nenhum registro (usuário nunca teve plano)
                if (error.code === 'PGRST116') {
                    console.log('ℹ️ Usuário sem histórico de planos, retornando free');
                    return {
                        plan: null,
                        subscription: null,
                        hasActivePlan: false,
            ...
            // ✅ Considerar expirado se:
            // 1. Status no banco é 'expired' OU 'cancelled', OU
            // 2. Data de vencimento já passou
            const isExpired =
                status === 'expired' ||
                status === 'cancelled' ||
                (!!endDateIso && this.isPlanExpiredByDate(endDateIso));

            console.log('🔍 getUserPlanSnapshot - Verificação de expiração:', {
                status,
                endDateIso,
                isPlanExpiredByDate: !!endDateIso && this.isPlanExpiredByDate(endDateIso),
                isExpired
            });
    ...
    static async getUserAdCounts(userId) {
        try {
            const { data, error } = await supabase
                .from('properties')
                .select('status, ad_status', { count: 'exact' })
                .eq('user_id', userId)
            ...
            const counts = { 
                ...
                pending: 0, 
                rejected: 0,
                approvedActive: 0 // ✨ NOVO: anúncios aprovados E ativos
            };
            ...
            (data || []).forEach((row) => {
                ...
                // Contar anúncios aprovados E ativos
                if (row.status === 'approved' && row.ad_status === 'active') {
                    counts.approvedActive += 1;
                }
            });
            ...
            return { approved: 0, pending: 0, rejected: 0, total: 0, approvedActive: 0 };
        }
    }
    ...
    static async userCanManageAds(userId) {
        try {
            ...
            // 1. Tem plano (gratuito ou pago)
            // 2. Plano NÃO está vencido (planos gratuitos não expiram)
            ...
            if (isFreePlan) {
                // Plano gratuito pode gerenciar se tiver anúncios
                canManageAds = hasAds;
                reason = hasAds
                    ? 'Pode gerenciar anúncios'
                    : 'Você ainda não tem anúncios para gerenciar';
    ...
    static async getUserEligibility(userId) {
        try {
            ...
            // Regras:
            // - Plano gratuito (max_ads = 1) => pode criar se total < max_ads
    ...
    // Contar anúncios inativos do usuário
    static async countInactiveAds(userId) {
        try {
            console.log('🔍 Contando anúncios inativos para userId:', userId);
            ...
            if (error) {
                console.error('❌ Erro ao contar anúncios inativos:', error);
                throw error;
            }

            console.log('✅ Total de anúncios inativos:', count || 0);
            return count || 0;
        } catch (error) {
            console.error('❌ Erro ao contar anúncios inativos:', error);
            return 0;
    ...
    // Buscar anúncios inativos do usuário (para seleção no modal)
    static async getInactiveAds(userId) {
        try {
            console.log('🔍 Buscando anúncios inativos para userId:', userId);

            const { data, error } = await supabase
                .from('properties')
                .select('id, title, price, created_at, images, ad_id')
                .eq('user_id', userId)
                .eq('ad_status', 'inactive')
                .order('created_at', { ascending: false });

            if (error) {
                console.error('❌ Erro ao buscar anúncios inativos:', error);
                throw error;
            }

            console.log('✅ Anúncios inativos encontrados:', data?.length || 0);
            return data || [];
        } catch (error) {
            console.error('❌ Erro ao buscar anúncios inativos:', error);
            return [];
    ...
    // Validar se usuário pode contratar plano (verificando anúncios inativos)
    static async validatePlanSelection(userId, targetPlanMaxAds) {
        try {
            console.log('🔍 Validando seleção de plano:', { userId, targetPlanMaxAds });

            // Contar anúncios inativos
            const inactiveAdsCount = await this.countInactiveAds(userId);

            // Verificar se o número de anúncios inativos excede o limite do plano
            if (inactiveAdsCount > targetPlanMaxAds) {
                return {
                    canProceed: false,
                    reason: `Você possui ${inactiveAdsCount} anúncios inativos, mas o plano selecionado permite apenas ${targetPlanMaxAds}. Por favor, exclua ${inactiveAdsCount - targetPlanMaxAds} anúncio(s) inativo(s) antes de contratar este plano.`,
                    inactiveAdsCount,
                    targetPlanMaxAds,
                    adsToRemove: inactiveAdsCount - targetPlanMaxAds
                };
            }
            ...
            // Em caso de erro, permitir prosseguir (fail-safe)
            return {
                canProceed: true,
                reason: 'Erro na validação, prosseguindo...',
                inactiveAdsCount: 0,
                targetPlanMaxAds: 0
            };
        }
