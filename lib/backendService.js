...
const BackendService = {
    ...
    // Verificar receipt Apple IAP (iOS)
    verifyIAPReceipt: async (payload) => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/iap/ios/verify`, {
                method: 'POST',
            ...
            if (!response.ok) {
                ...
                throw new Error(err.error || 'Erro ao verificar compra');
            }

            return await response.json();
        } catch (error) {
            console.error('❌ Erro verifyIAPReceipt:', error);
            throw error;
        }
    },
