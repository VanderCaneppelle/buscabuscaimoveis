...
-- Adicionar campos necessários para o sistema de aprovação

-- Adicionar colunas de aprovação na tabela properties
...
-- Atualizar RLS (Row Level Security) para o sistema de aprovação
...
-- Função para obter estatísticas de anúncios (apenas para admins)
...
-- Trigger para atualizar approved_at quando status for aprovado
...
-- Adicionar coluna is_admin na tabela profiles (se não existir)
...
-- Exemplo de como tornar um usuário admin (execute manualmente)
