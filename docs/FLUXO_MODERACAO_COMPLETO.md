...
Documentação completa do fluxo de aprovação e rejeição de propriedades no sistema Busca Busca Imóveis, incluindo notificações in-app e push.
    ...
    try {
        ...
        const { error: notificationError } = await supabase
            ...
            .insert({
                ...
                message: `Seu anúncio "${data.title}" foi aprovado e agora está visível para todos!`,
                data: {
        ...
        if (notificationError) {
            console.error('❌ APPROVE - Erro ao criar notificação in-app:', notificationError);
        } else {
        ...
        try {
            ...
            if (pushResponse.ok) {
                ...
                console.error('❌ APPROVE - Erro ao enviar push notification:', pushResponse.status);
            }
        } catch (pushError) {
            console.error('❌ APPROVE - Erro ao enviar push notification:', pushError);
        }
...
async function handler(req, res) {
    try {
        ...
        if (notificationError) {
            console.error('❌ REJECT - Erro ao criar notificação in-app:', notificationError);
        } else {
        ...
        try {
            ...
            if (pushResponse.ok) {
                ...
                console.error('❌ REJECT - Erro ao enviar push notification:', pushResponse.status);
            }
        } catch (pushError) {
            console.error('❌ REJECT - Erro ao enviar push notification:', pushError);
        }
